...
json_path = os.path.join(script_dir, 'point_groups.json')
...
try:
    ...
    CREATE TABLE IF NOT EXISTS point_groups (
        ...
        all_norm_matrices TEXT NOT NULL,
        generator_numbers TEXT NOT NULL,
        map_sets TEXT NOT NULL
    )
    ''')
    
    # Reset existing rows before import.
    cursor.execute('DELETE FROM point_groups')
    
    # Insert all rows.
    for group in data:
        cursor.execute('''
        INSERT INTO point_groups (id, all_matrices, all_norm_matrices,generator_numbers,map_sets)
        VALUES (?, ?, ?, ?, ?)
        ''', (
            group['point_group_id'],
            json.dumps(group['all_matrices']),
            json.dumps(group['all_norm_matrices']),
            json.dumps(group['generator_numbers']),
            json.dumps(group['map_sets'])
        ))
    
    conn.commit()
    conn.close()
    print(f"Database created successfully at {os.path.abspath(db_path)}")
    print(f"Imported {len(data)} point-group records")
