...
def _spin_texture_config_record(payload: dict) -> dict:
    ...
    if basis:
        basis_vectors = [
            _split_basis_remainder_suffix(expression)[0]
            for expression in combine_spin_texture_basis(basis)
        ]
        record["basis_vectors"] = basis_vectors
        record["basis_vectors_latex"] = spin_texture_basis_latex(basis_vectors)
        basis_span = combine_spin_texture_basis_span(basis_vectors)
        record["basis"] = _append_basis_remainder_ascii(basis_span, remainder_order)
